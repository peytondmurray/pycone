...
the problem; the `emcee` sampler can only efficiently explore the parameter space if the
step size used to generate samples is appropriate for all parameters

#figure(
  image("mcmc_diagram.svg"),
  caption: [
    Markov chains for each fit parameter generated by `emcee`. Initially the chains vary
    as the MCMC sampler searches the parameter space of the problem; eventually they fall
    into a region of instability, indicating that the model probably needs to be
    reparameterized.
  ]
)
